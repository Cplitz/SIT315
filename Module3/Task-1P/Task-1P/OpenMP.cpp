...
int openmp_run()
{
	...
	// Master initialise matrices A and B with random values
	if (rank == 0) {
		printf("+--------------------------+\n");
	...
	#pragma omp parallel for shared(rcvA, b, partC) private(i, j, k) collapse(3) num_threads(NUM_THREADS)
	...
	// Master processes received parts (prints values)
	if (rank == 0) {
		exec_end = clock();
